...
class Car(Transport):
    ...
    def init(self, model, year, color, penalties=0):
        # fields / attributes
        super().init(model, year, color)
...
class Truck(Car):
    counter = 0
    def init(self, model, year, color, load_capacity, penalties=0):
        super().init(model, year, color, penalties)
